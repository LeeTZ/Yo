...
let rec build_stmt_semantic ctx = function
			...
			| Some e -> 
				...
				(* When the l-value is defined, check its type against that of r-value *)
					...
					if  ltype = rtype 
						...
						else raise (SemanticError ("Invalid assignment: expecting " ^ ltype.name 
							^ ", but having " ^ rtype.name ))
	...
	| ForIn (varname, expr, stmts) -> 
		...
		let arr_ele_type = try extract_arr_ele_type ctx (extract_semantic s_expr).type_def 
							with Invalid_argument _ -> raise (SemanticError ((string_of_expr expr)
	...
	| ForRange (varname, st_expr, ed_expr, stmts, dir) ->
		...
		let svar = new_var nested_env varname 
			(if (extract_semantic s_st_expr).type_def.name = "Int" 
			then let ed_type=(extract_semantic s_ed_expr).type_def in
			 (if ed_type.name = "Int" then ed_type 
...
let rec build_type_mem_semantic ctx = function
			...
			| MemVarDecl  memvar  -> 
				...
				| VarDecl (varname, vartype) -> SMemVarDecl (SVarDecl (varname, 
					{actions=[]; type_def=look_up_type vartype ctx.typetab}))
